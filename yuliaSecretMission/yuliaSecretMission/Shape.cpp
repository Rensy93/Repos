...
Shape::Shape(const Shape& original)
{
    ...
    this->coord=original.coord->clone();
    this->box=original.box->clone();
...
bool Shape::intersectsWith(const Shape &otherShape)
{
    ...
    if((rectMaxWidth>= otherRectMinWidth &&  rectMinWidth <=otherRectMaxWidth)
       && (rectMaxHeight>= otherRectMinHeight && rectMinHeight<= otherRectMaxHeight))
